...
d_z_standard = a_0/4 
Z = 16 # range of simulation, - 10 nm <z< 10 nm 
r = 2 
...
def n(z, w = w,lbda = lbda, no_wiggle = False):
    if no_wiggle: 
...
def V_l(w=w, lbda = lbda, no_wiggle = False):
    return E_G * n(z_values, w, lbda, no_wiggle = no_wiggle )
...
def get_gs_fes(z_values, well_potential, disorder, electric_noise, electric_potential ):
    length = len(z_values)
    t = - hbar**2 / 2 / m_l / d_z**2 * 10**18
    ...
    ground_state = V[:, 0]
    first_excited_state = V[:, 1]
    energy_gap = D[1] - D[0]
    return ground_state, first_excited_state, energy_gap
...
def expectation_value_2(term, bra, ket):
    val = np.dot(bra, term * ket)
    return val

def calculate_Rabi_freq(Delta_0000, Delta_1000, beta, mode = 'simple'):
    coeff = E * g0 * mu_b * B / (hbar*(hbar * omega_t)**2)
    if mode == 'simple':
...
def calculate_Zeeman_splitting(theta, g_tau, Delta_0000):
    # calculate normalized Zeeman splitting 
    phi_v = Delta_0000[1]
    g_tau, phi_g = g_tau
    return g0 * np.sin(theta) - g_tau  * np.cos(theta) * np.cos(phi_v - phi_g)    


def calculate_g_tau(i_k_z_01, beta_10, gap):
    g_tau = - hbar  / mu_b/m_l * 10 **3 
    g_tau = g_tau * (i_k_z_01* beta_10)/ (gap + hbar * omega_t) # just first term 
    return np.abs(g_tau), np.angle(g_tau)